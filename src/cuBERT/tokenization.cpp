...
void FullTokenizer::convert_tokens_to_ids(
    const std::vector<std::string> &tokens, uint64_t *ids) {
...
void BasicTokenizer::tokenize(const char *text,
    ...
    while (word_bytes > 0) {
        int len = utf8proc_iterate((const utf8proc_uint8_t *)text + subpos,
                                   word_bytes, &cp);
        if (len < 0) {
        ...
        if (cp == 0 || cp == 0xfffd || _is_control(cp, cat)) {
            ...
            if (_is_punctuation(cp, cat) || _is_chinese_char(cp)) {
                ...
                if (new_token) {
                    ...
                    output_tokens->at(output_tokens->size() - 1)
                        .append(dst_ptr, dst_len);
                }
            }
        }
...
void WordpieceTokenizer::tokenize(const std::string &token,
                                  std::vector<std::string> *output_tokens) {
    ...
    for (size_t start = 0; start < token.size();) {
        ...
        for (size_t end = token.size(); start < end;
             --end) { // FIXME: slightly different
