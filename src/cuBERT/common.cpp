...
#ifdef HAVE_CUDA
    CUDA_CHECK(cudaMemcpyAsync(dst, src, n, (cudaMemcpyKind)kind,
                               (cudaStream_t)stream));
#else
    std::memcpy(dst, src, n);
#endif
...
template <>
void blas_gemm<float>(void *handle, const bool TransA, const bool TransB,
                      const int M, const int N, const int K, const float alpha,
                      const float *A, const int lda, const float *B,
                      const int ldb, const float beta, float *C, const int ldc,
                      int algo) {
#ifdef HAVE_CUDA
    std::cout << "cublassgemm: " << TransA << " " << TransB << std::endl;
    CUBLAS_CHECK(
        cublasGemmEx((cublasHandle_t)handle, TransA ? CUBLAS_OP_T : CUBLAS_OP_N,
                     TransB ? CUBLAS_OP_T : CUBLAS_OP_N, M, N, K, &alpha, A,
                     CUDA_R_32F, lda, B, CUDA_R_32F, ldb, &beta, C, CUDA_R_32F,
                     ldc, CUDA_R_32F, (cublasGemmAlgo_t)algo));
    return;
#endif
#ifdef HAVE_MKL
...
template <>
void blas_gemm_batch<float>(void *handle, const bool TransA, const bool TransB,
                            int m, int n, int k, const float alpha,
                            const float **Aarray, int lda, const float **Barray,
                            int ldb, const float beta, float **Carray, int ldc,
                            int batchCount, int algo) {
#ifdef HAVE_CUDA
    std::cout << "!!!! cublasbatchedgemm: " << TransA << " " << TransB
              << std::endl;
    // CUBLAS_CHECK(cublasGemmBatchedEx((cublasHandle_t)handle,
    //                                  TransA ? CUBLAS_OP_T : CUBLAS_OP_N,
    //                                  TransB ? CUBLAS_OP_T : CUBLAS_OP_N, m,
    //                                  n, k, &alpha, (const void **)Aarray,
    //                                  CUDA_R_32F, lda, (const void **)Barray,
    //                                  CUDA_R_32F, ldb, &beta, (void **)Carray,
    //                                  CUDA_R_32F, ldc, batchCount, CUDA_R_32F,
    ...
    cblas_sgemm_batch(CblasColMajor, &transA, &transB, &m, &n, &k, &alpha,
                      Aarray, &lda, Barray, &ldb, &beta, Carray, &ldc, 1,
                      &batchCount);
    return;
#endif
}
...
template <>
void blas_gemm_strided_batch<float>(
    void *handle, const bool TransA, const bool TransB, int m, int n, int k,
    const float alpha, const float *A, int lda, long long int strideA,
    const float *B, int ldb, long long int strideB, const float beta, float *C,
    int ldc, long long int strideC, int batchCount, int algo) {
#ifdef HAVE_CUDA
    CUBLAS_CHECK(cublasGemmStridedBatchedEx(
        (cublasHandle_t)handle, TransA ? CUBLAS_OP_T : CUBLAS_OP_N,
        TransB ? CUBLAS_OP_T : CUBLAS_OP_N, m, n, k, &alpha, A, CUDA_R_32F, lda,
        strideA, B, CUDA_R_32F, ldb, strideB, &beta, C, CUDA_R_32F, ldc,
        strideC, batchCount, CUDA_R_32F, (cublasGemmAlgo_t)algo));
    return;
#endif
#ifdef HAVE_MKL
...
#ifdef HAVE_CUDA
template <>
void blas_gemm<half>(void *handle, const bool TransA, const bool TransB,
                     const int M, const int N, const int K, const float alpha,
                     const half *A, const int lda, const half *B, const int ldb,
                     const float beta, half *C, const int ldc, int algo) {
    CUBLAS_CHECK(
        cublasGemmEx((cublasHandle_t)handle, TransA ? CUBLAS_OP_T : CUBLAS_OP_N,
                     TransB ? CUBLAS_OP_T : CUBLAS_OP_N, M, N, K, &alpha, A,
                     CUDA_R_16F, lda, B, CUDA_R_16F, ldb, &beta, C, CUDA_R_16F,
                     ldc, CUDA_R_32F, (cublasGemmAlgo_t)algo));
}

template <>
void blas_gemm_batch<half>(void *handle, const bool TransA, const bool TransB,
                           int m, int n, int k, const float alpha,
                           const half **Aarray, int lda, const half **Barray,
                           int ldb, const float beta, half **Carray, int ldc,
                           int batchCount, int algo) {
    CUBLAS_CHECK(cublasGemmBatchedEx(
        (cublasHandle_t)handle, TransA ? CUBLAS_OP_T : CUBLAS_OP_N,
        TransB ? CUBLAS_OP_T : CUBLAS_OP_N, m, n, k, &alpha,
        (const void **)Aarray, CUDA_R_16F, lda, (const void **)Barray,
        CUDA_R_16F, ldb, &beta, (void **)Carray, CUDA_R_16F, ldc, batchCount,
        CUDA_R_32F, (cublasGemmAlgo_t)algo));
}

template <>
void blas_gemm_strided_batch<half>(
    void *handle, const bool TransA, const bool TransB, int m, int n, int k,
    const float alpha, const half *A, int lda, long long int strideA,
    const half *B, int ldb, long long int strideB, const float beta, half *C,
    int ldc, long long int strideC, int batchCount, int algo) {
    std::cout << "!!!!! strided batch gemm: " << TransA << " " << TransB
              << std::endl;
    CUBLAS_CHECK(cublasGemmStridedBatchedEx(
        (cublasHandle_t)handle, TransA ? CUBLAS_OP_T : CUBLAS_OP_N,
        TransB ? CUBLAS_OP_T : CUBLAS_OP_N, m, n, k, &alpha, A, CUDA_R_16F, lda,
        strideA, B, CUDA_R_16F, ldb, strideB, &beta, C, CUDA_R_16F, ldc,
        strideC, batchCount, CUDA_R_32F, (cublasGemmAlgo_t)algo));
}
#endif

template <> void T2T<float, float>(const float *in, float *out, size_t n) {
    std::memcpy(out, in, n * sizeof(float));
}

template <> void T2T<half, half>(const half *in, half *out, size_t n) {
    std::memcpy(out, in, n * sizeof(half));
}

template <> void T2T<float, half>(const float *in, half *out, size_t n) {
    float2half(in, out, n);
}

template <> void T2T<half, float>(const half *in, float *out, size_t n) {
    half2float(in, out, n);
}

template <> int gemm_algo<float>(const char *env) {
...
template <> int gemm_algo<half>(const char *env) {
    char *val = std::getenv(env);
    if (val == nullptr) {
#ifdef HAVE_CUDA
