...
class cuBertTest : public ::testing::Test {
  protected:
    void SetUp() override { cuBERT_initialize(); }

    void TearDown() override { cuBERT_finalize(); }
};

void random_input(int *input_ids, int8_t *input_mask, int8_t *segment_ids,
                  size_t length) {
    std::random_device r;
...
TEST_F(cuBertTest, compute) {
    ...
    void *model =
        cuBERT_open("bert_frozen_seq32.pb", max_batch_size, seq_length, 12, 12);
    std::cout << "start compute" << std::endl;
    auto st = std::chrono::system_clock::now();
    cuBERT_compute(model, batch_size, input_ids, input_mask, segment_ids,
                   logits);
...
//     void* model = cuBERT_open("bert_frozen_seq32.pb", max_batch_size,
//     seq_length, 12, 12); void* tokenizer =
//     cuBERT_open_tokenizer("vocab.txt");
...
//     void* model = cuBERT_open("bert_frozen_seq32.pb", max_batch_size,
//     seq_length, 12, 12, cuBERT_COMPUTE_HALF); void* tokenizer =
//     cuBERT_open_tokenizer("vocab.txt");
